...
impl Conf {
    ...
    pub fn get_str(&self, key: &str) -> Option<&str> {
        unsafe {
            let c_key =
                CString::new(key).expect("`key` contains a stray NUL byte");
    ...
    pub fn set_str(&mut self, key: &str, value: &str) {
        unsafe {
            let c_key =
                CString::new(key).expect("`key` contains a stray NUL byte");
            let c_value =
                CString::new(value).expect("`value` contains a stray NUL byte");
